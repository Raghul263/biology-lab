...
const StainBeaker = ({ position = [0, 0, 0] }) => {
  ...
  const handleClick = (e) => {
    ...
    if (currentStep === STEPS.CHEMICAL_TREAT && heldTool === 'dropper') {
      ...
      if (hclAdded && !stainAdded) {
        ...
        showWrongAction('Add HCl first before the stain.');
      } else {
        showWrongAction('Stain already added to the slide.');
      }
    }
  };
  ...
  const isActive = (() => {
    const { hclAdded, stainAdded } = useStore.getState();
    return currentStep === STEPS.CHEMICAL_TREAT && heldTool === 'dropper' && hclAdded && !stainAdded;
  })();
