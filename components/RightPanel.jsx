...
const RightPanel = () => {
  const { currentStep, observations, addObservation } = useStore();
  ...
  const handleSave = () => {
    ...
    addObservation({
      id: Date.now(),
      number: (observations?.length || 0) + 1,
      stage,
      notes: notes.trim(),
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: '9px', letterSpacing: '3px', color: '#ce93d8', marginBottom: '4px', fontWeight: 600 }}>
          LAB RECORD
        </div>
      ...
      <div style={{ padding: '14px', borderBottom: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
        ...
        <label style={{ display: 'block', fontSize: '11px', color: '#ce93d8', marginBottom: '5px', fontWeight: 600 }}>
          Observed Stage
        </label>
        <select
          value={stage}
          ...
          style={{
            ...
            marginBottom: '10px',
            outline: 'none',
            cursor: canObserve ? 'pointer' : 'not-allowed',
            opacity: canObserve ? 1 : 0.5,
          }}
        >
          {STAGES.map((s, i) => (
        ...
        <textarea
          ...
          style={{
            ...
            opacity: canObserve ? 1 : 0.5,
            cursor: canObserve ? 'text' : 'not-allowed',
          }}
        />

        <button
          onClick={handleSave}
          disabled={!canObserve || !notes.trim()}
          style={{
            width: '100%',
            marginTop: '10px',
            padding: '10px',
            background: canObserve && notes.trim()
          ...
          [...(observations || [])].reverse().map((obs) => (
            <div key={obs.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                <span style={{ fontSize: '11px', fontWeight: 700, color: '#ce93d8' }}>#{obs.number} {obs.stage}</span>
