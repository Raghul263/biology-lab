...
const OUTLET_WORLD = new THREE.Vector3(1.6, 0.93, 0.3);

const Burner = ({ position: initialPosition = [1.4, 0.93, 0.3] }) => {
  ...
  useFrame((state) => {
    ...
    // 2. Dynamic Hose Stretching
    if (groupRef.current && hoseRef.current) {
        const localOutlet = groupRef.current.worldToLocal(OUTLET_WORLD.clone());
        const curve = new THREE.CatmullRomCurve3([
          new THREE.Vector3(0, 0.05, -0.02),
          new THREE.Vector3(localOutlet.x * 0.3, 0.01, localOutlet.z * 0.3 - 0.05),
          new THREE.Vector3(localOutlet.x * 0.7, 0.0, localOutlet.z * 0.7),
          localOutlet
        ]);
        const newGeo = new THREE.TubeGeometry(curve, 24, 0.007, 8, false);
        if (hoseRef.current.geometry) hoseRef.current.geometry.dispose();
        hoseRef.current.geometry = newGeo;
    }
    ...
    if (burnerOn && groupRef.current) {
        ...
        if (distToWG < 0.25) {
          ...
          if (store.watchGlassFluid === 'HCL' && newTime > 150) updates.rootProcessingState = 'MACERATED';
          else if (store.watchGlassFluid === 'STAIN' && newTime > 300) updates.rootProcessingState = 'STAINED';
          setStates(updates);
        }
    }
  });
  ...
  return (
    ...
    >
      ...
      {/* 🌫️ DYNAMIC GAS HOSE */}
      <mesh ref={hoseRef} castShadow>
        <meshStandardMaterial color="#111111" roughness={1.0} metalness={0} />
      </mesh>
        ...
        <group position={[0, 0.27, 0]}>
          <group ref={flameRef}>
            <mesh position={[0, 0.05, 0]}>
              ...
              <meshStandardMaterial color="#ff9800" transparent opacity={0.6} side={THREE.DoubleSide} emissive="#ff5722" emissiveIntensity={2.5} />
            </mesh>
            <mesh position={[0, 0.04, 0]}>
              <cylinderGeometry args={[0.001, 0.008, 0.08, 12, 8, true]} />
              <meshStandardMaterial color="#ffeb3b" transparent opacity={0.8} side={THREE.DoubleSide} emissive="#ffeb3b" emissiveIntensity={4} />
            </mesh>
            <mesh position={[0, 0.015, 0]}>
              <cylinderGeometry args={[0.006, 0.012, 0.03, 12, 1, true]} />
              <meshStandardMaterial color="#00e5ff" transparent opacity={0.6} side={THREE.DoubleSide} emissive="#00e5ff" emissiveIntensity={1.5} />
            </mesh>
          </group>
