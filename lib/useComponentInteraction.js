...
export function useComponentInteraction(id, isHeld, options = {}) {
  ...
  const handlePointerDown = (e) => {
    ...
    const isInteractionAttempt = activeTool && activeTool !== id;

    // If it's an interaction and we don't have a specific handler for it,
    // don't stop propagation so the held tool can catch its own drop event.
    if (isInteractionAttempt && !options.onInteraction && !isLocked) {
      return;
    }
    ...
    if (isHeld) {
      ...
      if (isInteractionAttempt) {
        // Double check interaction attempt (redundant but safe)
        if (options.onInteraction) options.onInteraction(e);
        return;
      }
